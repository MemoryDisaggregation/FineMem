...
public:
    ...
    bool malloc(mralloc::mr_rdma_addr &remote_addr) override {
        // current_index_ += mt();
        int retry_time = conn_[rr%mem_node_num]->fetch_block(current_index_, remote_addr.addr, remote_addr.rkey, remote_addr.size);
        remote_addr.node = rr%mem_node_num;
        rr ++;
        if(retry_time) {
    ...
    bool free(mralloc::mr_rdma_addr remote_addr) override {
        return conn_[remote_addr.node]->free_block(remote_addr.addr, remote_addr.size);
    };
...
public:
    ...
    bool malloc(mralloc::mr_rdma_addr &remote_addr) override {
        ...
        while(conn_[rr%mem_node_num]->remote_fetch_block(remote_addr.addr, remote_addr.rkey, remote_addr.size)); 
        // printf("%lx\n", remote_addr.addr);
        remote_addr.node = rr%mem_node_num;
        rr++;
	    return true;
    ...
    bool free(mralloc::mr_rdma_addr remote_addr) override {
        ...
        while(conn_[remote_addr.node]->remote_free_block(remote_addr.addr));
	    return true;
    };
...
public:
    ...
    ~rpc_allocator() {};
    bool malloc(mralloc::mr_rdma_addr &remote_addr) override {
        conn_[rr%mem_node_num]->register_remote_memory(remote_addr.addr, remote_addr.rkey, REMOTE_MEM_SIZE*1<<remote_addr.size);
        remote_addr.node = rr%mem_node_num;
        rr++;
        return true;
    };
    bool free(mralloc::mr_rdma_addr remote_addr) override {
        return !conn_[remote_addr.node]->unregister_remote_memory(remote_addr.addr);
    };
    bool print_state() override {return false;};
private:
    mralloc::ConnectionManager** conn_;
...
public:
    share_allocator(mralloc::ConnectionManager** conn, uint64_t start_hint) {
        conn_ = conn;
        skip_mask = 0;
        for(int i =0; i < mem_node_num; i++){
            cache_region_index[i] = start_hint;
            conn_[i]->find_section(cache_section[i], cache_section_index[i], alloc_class, mralloc::alloc_light);
        }
        // conn_->fetch_region(cache_section, cache_section_index, alloc_class, false, cache_region, cache_region_index, skip_mask);
    }
    ~share_allocator() {};
    bool malloc(mralloc::mr_rdma_addr &remote_addr) override {
        int retry_time = conn_[rr%mem_node_num]->full_alloc(cache_section[rr%mem_node_num], cache_section_index[rr%mem_node_num], remote_addr.size, remote_addr.addr, remote_addr.rkey);
        remote_addr.node = rr%mem_node_num;
        rr++;
        return true;
    }
    bool free(mralloc::mr_rdma_addr remote_addr) override {
        int result = conn_[remote_addr.node]->full_free(remote_addr.addr, remote_addr.size);
        return true;
    };
...
private:
    ...
    uint32_t cache_section_index[32];
    uint32_t cache_region_index[32];
    uint32_t skip_mask;
    uint16_t alloc_class = 6;
    mralloc::section_e cache_section[32];
    mralloc::region_e cache_region[32];
    mralloc::ConnectionManager** conn_;
    int rr;
};
...
void* worker(void* arg) {
    ...
    {
    case stage_test:
        stage_alloc(conn[0], alloc, thread_id);
        break;
    case shuffle_test:
        shuffle_alloc(conn[0], alloc, thread_id);
        break;
    case short_test:
